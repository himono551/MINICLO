...
const INVENTORY_HEADERS = [
  ...
  'category2', 'remaining', 'memo', 'purchase_date', 'price', 'capacity', 'url', 'tags', 'image_url',
  'qty', 'created_at', 'updated_at',
];
const MANUAL_CAPTURE_HEADERS = [
  'created_at', 'source', 'site', 'url', 'image_url', 'name', 'brand', 'type', 'category',
  'category2', 'status', 'color', 'purchase_date', 'price', 'memo', 'raw_text',
];
...
function doPost(e) {
  try {
    ...
    const paramList = (e && e.parameters) ? e.parameters : {};
    const action = String(
      body.action
      || params.action
      || (paramList.action && paramList.action[0])
      || ''
    ).trim();
...
function appendManualItem_(payload) {
  ...
  const normalizedStatus = /ほしい|欲しい|want|wish|approved|hold/i.test(statusRaw)
    ? 'ほしい'
    : (/手放す|dispose|drop|discard|disposed|dropped/i.test(statusRaw) ? '手放す' : 'もってる');
  ...
  const row = headers.map(function(header) {
    ...
    if (normalized === 'status') return normalizedStatus;
...
function purgeUnusedSheets_() {
  deleteSheetIfExists_(SHEET_NAMES.wishlistItems);
  deleteSheetIfExists_(SHEET_NAMES.behaviorEvents);
}
...
function loadData_() {
  ...
  const hasData = categories.length || inventoryItems.length || deletedInventoryItems.length || budgetMonths.length;
  if (hasData) {
    return {
      categories: categories,
      inventoryItems: inventoryItems,
